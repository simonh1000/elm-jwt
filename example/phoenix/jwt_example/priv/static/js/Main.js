...
Elm.Jwt.make = function (_elm) {
   ...
   var decodeToken = F2(function (dec,s) {
      var f1 = A2($String.split,".",unurl(s));
      var f2 = A2($List.map,fixlength,f1);
      var _p2 = f2;
      _v2_2: do {
         if (_p2.ctor === "::" && _p2._1.ctor === "::") {
               if (_p2._1._0.ctor === "Err") {
                     if (_p2._1._1.ctor === "::" && _p2._1._1._1.ctor === "[]") {
                           return $Result.Err(_p2._1._0._0);
                        } else {
                           break _v2_2;
                        }
                  } else {
                     if (_p2._1._1.ctor === "::" && _p2._1._1._1.ctor === "[]") {
                           var _p3 = $Base64.decode(_p2._1._0._0);
                           if (_p3.ctor === "Ok") {
                                 var _p4 = A2($Json$Decode.decodeString,dec,_p3._0);
                                 if (_p4.ctor === "Ok") {
                                       return $Result.Ok(_p4._0);
                                    } else {
                                       return $Result.Err(TokenDecodeError(_p4._0));
                                    }
                              } else {
                                 return $Result.Err(TokenProcessingError(_p3._0));
                              }
                        } else {
                           break _v2_2;
                        }
...
Elm.App.make = function (_elm) {
   ...
   var TryToken = {ctor: "TryToken"};
   var Submit = {ctor: "Submit"};
   var Input = F2(function (a,b) {    return {ctor: "Input",_0: a,_1: b};});
   var Data2 = function (a) {    return {ctor: "Data2",_0: a};};
   var Data = function (a) {    return {ctor: "Data",_0: a};};
   var Model = F4(function (a,b,c,d) {    return {uname: a,pword: b,token: c,errorMsg: d};});
   var init = {ctor: "_Tuple2",_0: A4(Model,"testuser","testpassword",$Maybe.Nothing,""),_1: $Effects.none};
   var JwtToken = F4(function (a,b,c,d) {    return {id: a,username: b,iat: c,expiry: d};});
   var tokenDecoder = A5($Json$Decode.object4,
   JwtToken,
   A2($Json$Decode._op[":="],"aud",$Json$Decode.string),
   A2($Json$Decode._op[":="],"iss",$Json$Decode.string),
   A2($Json$Decode._op[":="],"iat",$Json$Decode.$int),
   A2($Json$Decode._op[":="],"exp",$Json$Decode.$int));
   var Pword = {ctor: "Pword"};
   var Uname = {ctor: "Uname"};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("container")]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("elm-jwt in action")]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text("username = testuser, password = testpassword")]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("row")]),
              _U.list([A2($Html.form,
              _U.list([A2(onSubmit$,address,Submit),$Html$Attributes.$class("col s12")]),
              _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("row")]),
              _U.list([A2($Html.div,
                      _U.list([$Html$Attributes.$class("input-field col s12")]),
                      _U.list([A2($Html.input,
                              _U.list([A3($Html$Events.on,"input",A2($Json$Decode.map,Input(Uname),$Html$Events.targetValue),$Signal.message(address))
                                      ,$Html$Attributes.id("uname")
                                      ,$Html$Attributes.type$("text")
                                      ,$Html$Attributes.value(model.uname)]),
                              _U.list([]))
                              ,A2($Html.label,_U.list([$Html$Attributes.$for("uname")]),_U.list([$Html.text("Username")]))]))
                      ,A2($Html.div,
                      _U.list([$Html$Attributes.$class("input-field col s12")]),
                      _U.list([A2($Html.input,
                              _U.list([A3($Html$Events.on,"input",A2($Json$Decode.map,Input(Pword),$Html$Events.targetValue),$Signal.message(address))
                                      ,$Html$Attributes.id("pword")
                                      ,$Html$Attributes.type$("password")
                                      ,$Html$Attributes.value(model.pword)]),
                              _U.list([]))
                              ,A2($Html.label,_U.list([$Html$Attributes.$for("pword")]),_U.list([$Html.text("Password")]))]))
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([A2($Html.button,_U.list([$Html$Attributes.type$("submit")]),_U.list([$Html.text("Submit")]))]))]))]))]))
              ,A2($Html.p,
              _U.list([]),
              _U.list([$Html.text(_U.eq(model.token,$Maybe.Nothing) ? "" : $Basics.toString(A2($Jwt.decodeToken,
              tokenDecoder,
              A2($Maybe.withDefault,"",model.token))))]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text(model.errorMsg)]))
              ,A2($Html.button,
              _U.list([$Html$Attributes.$class("btn waves-effect waves-light"),A2($Html$Events.onClick,address,TryToken)]),
              _U.list([$Html.text("try token")]))]));
   });
   var server = "http://localhost:4000/";
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Data": var _p1 = _p0._0;
           if (_p1.ctor === "Ok") {
                 return {ctor: "_Tuple2",_0: _U.update(model,{token: $Maybe.Just(_p1._0),errorMsg: ""}),_1: $Effects.none};
              } else {
                 return {ctor: "_Tuple2",_0: _U.update(model,{errorMsg: $Basics.toString(_p1._0)}),_1: $Effects.none};
              }
         case "Data2": var _p2 = _p0._0;
           if (_p2.ctor === "Ok") {
                 return {ctor: "_Tuple2",_0: _U.update(model,{errorMsg: _p2._0}),_1: $Effects.none};
              } else {
                 return {ctor: "_Tuple2",_0: _U.update(model,{errorMsg: $Basics.toString(_p2._0)}),_1: $Effects.none};
              }
         case "Input": var _p4 = _p0._1;
           var res = function () {
              var _p3 = _p0._0;
              if (_p3.ctor === "Uname") {
                    return _U.update(model,{uname: _p4});
                 } else {
                    return _U.update(model,{pword: _p4});
                 }
           }();
           return {ctor: "_Tuple2",_0: res,_1: $Effects.none};
         case "Submit": var jsonString = A2($Json$Encode.encode,
           0,
           $Json$Encode.object(_U.list([{ctor: "_Tuple2",_0: "username",_1: $Json$Encode.string(model.uname)}
                                       ,{ctor: "_Tuple2",_0: "password",_1: $Json$Encode.string(model.pword)}])));
           return {ctor: "_Tuple2"
                  ,_0: model
                  ,_1: $Effects.task(A2($Task.map,
                  Data,
                  A3($Jwt.authenticate,A2($Json$Decode._op[":="],"token",$Json$Decode.string),A2($Basics._op["++"],server,"sessions"),jsonString)))};
         default: return {ctor: "_Tuple2"
                         ,_0: model
                         ,_1: $Effects.task(A2($Task.map,
                         Data2,
                         $Task.toResult(A3($Jwt.getWithJwt,
                         A2($Maybe.withDefault,"",model.token),
                         A2($Json$Decode._op[":="],"data",$Json$Decode.string),
                         A2($Basics._op["++"],server,"api/data")))))};}
   });
   return _elm.App.values = {_op: _op,init: init,update: update,view: view};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({init: $App.init,update: $App.update,view: $App.view,inputs: _U.list([])});
   var main = app.html;
